    ...
    <li class="u-comment-panel" v-for="(comment, key) in comments" :key="key">
      <div class="u-comment">
        ...
        <div class="u-nickname-panel">
          <a class="u-nickname" :href="comment.user_id ? author_url(comment.user_id) : null"
             target="_blank" v-text="comment.user_nickname"></a>
          <template v-if="comment.parent_id">
            <span>&nbsp;回复&nbsp;</span>
            <a class="u-nickname" :href="comment.parent.user_id ? author_url(comment.parent.user_id) : null"
               target="_blank" v-text="comment.parent.user_nickname"></a>
          </template>
        </div>
        ...
        <div class="m-reply">
          <!-- 展开、收起 -->
          <el-button type="default" v-if="comment.reply_form.show" class="u-reply"
                     @click="comment.reply_form.show = !comment.reply_form.show">
            <i class="el-icon-arrow-up"></i>
            <span>收起</span>
          </el-button>
          <el-button type="primary" v-else class="u-reply"
                     @click="comment.reply_form.show = !comment.reply_form.show">
